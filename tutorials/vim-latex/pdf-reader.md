...
- in the background, constantly listens for changes to the PDF document and automatically updates its display when the document’s contents change after compilation
  ...
  (Or I guess you could hack together a shell script, but why bother?)
...
### On macOS
On macOS, to the best of my knowledge, you have basically one option meeting the above three criteria: [Skim](https://skim-app.sourceforge.io/).
...
See also `help g:vimtex_view_method` for other possibilities on Windows.
...
**Note** Neovim will not create a listen address at `/tmp/texsocket` if a file `/tmp/texsocket` already exists there from a different LaTeX document.
...
In case you use GUI editor MacVim, basically everything is done for you.
In Skim, enable SyncTex in `Skim > Preferences > Sync > PDF-TeX Sync Support`.
...
- From the documentation in `:help clientserver` you *should* be able to
  ...
  If you are a Vim user struggling with this step, now might be as good a time as any to try out Neovim, but that's probably not too helpful.
